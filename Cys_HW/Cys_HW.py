...
from funct_Cys_HW import corr_plot
...
Nblocks = 10   # adjust if from corr time estimation tau is large
...
for suf in suffixes:
    ...
    # LOAD TIME SERIES
    # ============================
    data = np.loadtxt(f"5_Data_Analysis/dih_HW_time_{suf}.xvg", comments=["@", "#"])
    angles = data[:, 1]
    ...
    plt.xlabel(f"HW Dihedral χ_{suf} angle (degrees)")
    plt.ylabel("Probability")
    plt.xlim(-180, 180)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"images/dihedral_HW_histogram_{suf}.png", dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"images/free_energy_HW_{suf}.png", dpi=300)
    plt.close()
...
data_rdf_H = np.loadtxt("5_Data_Analysis/rdfD2O_HG_WH.xvg", comments=["@", "#"])
...
plt.tight_layout()
plt.savefig("images/rdfD2O_HG_WH.png", dpi=300)
plt.close()

data_rdf_S = np.loadtxt("5_Data_Analysis/rdfD2O_SG_WO.xvg", comments=["@", "#"])
...
plt.tight_layout()
plt.savefig("images/rdfD2O_SG_WO.png", dpi=300)
plt.close()
